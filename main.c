...
void sensor(){
	if(GPIOA->IDR & 1<<11){HA=1;
}else {HA=0;}
if(GPIOA->IDR & 1<<12){HB=1;
} else { HB=0;} 
if(GPIOA->IDR & 1<<13){ HC=1; 
} else { HC=0;
	if(GPIOA->IDR & 1<<14){HD=1;
} else { HD=0;} 
if(GPIOA->IDR & 1<<15){ HE=1; 
} else { HE=0;
}}}

void durum1(){
GPIOA->ODR &= ~ GPIO_ODR_OD6; // 101
...
GPIOA->MODER |= GPIO_IDR_ID11 | GPIO_IDR_ID12 | GPIO_IDR_ID13 | GPIO_IDR_ID14 | GPIO_IDR_ID15 | GPIO_IDR_ID2; //SENSOR VERILERINI ALMAK ICIN
